...
from config import OPENAI_API_KEY, OPENAI_MODEL, SENTIMENT_PROMPT, AgentSignal, DocumentContext
...
async def run_sentiment_agent(
    ...
    user_message = f"""Analyse news sentiment for: {ticker.upper()}
Use search_news and get_stock_info tools.
{extra_context}
Return JSON only: {{"score": 0-100, "confidence": 0-100, "summary": "...", "data_points": [...], "source": "live"}}"""

    servers = []
    try:
        for srv in get_servers_for_agent("sentiment"):
            await srv.__aenter__()
            servers.append(srv)
        agent = Agent(name="Sentiment Agent", instructions=SENTIMENT_PROMPT,
        ...
        return AgentSignal(agent="sentiment", score=50.0, confidence=5.0,
                           summary=f"Sentiment agent failed: {e}", data_points=[], source="error")
    finally:
        for srv in servers:
    ...
    return AgentSignal(
        ...
        confidence=float(out.get("confidence", 40)),
        summary=out.get("summary", "Sentiment analysis completed."),

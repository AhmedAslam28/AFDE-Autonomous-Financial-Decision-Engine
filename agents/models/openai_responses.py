...
try:
    from openai.types.responses.response_prompt_param import ResponseInputParam
    ResponsePromptParam = ResponseInputParam
except ImportError:
    ResponsePromptParam = dict  # Fallback for ResponseInputParam
...
class OpenAIResponsesModel(Model):
    ...
    async def _fetch_response(
        ...
        # Prepare core parameters (always supported)
        core_params = {
        ...
        # Optional parameters that may not be supported in older versions
        optional_params = {}
        if conversation_id is not None:
            optional_params["conversation"] = conversation_id
        if prompt is not None:
            optional_params["prompt"] = prompt

        # Try with all parameters first (newest OpenAI versions)
        if optional_params:
            try:
                logger.debug(f"Attempting to call responses.create with optional params: {list(optional_params.keys())}")
                return await self._client.responses.create(**core_params, **optional_params)
            except TypeError as e:
                error_msg = str(e)
                logger.debug(f"TypeError with optional params: {error_msg}")
                
                # Try removing specific problematic parameters
                if "conversation" in error_msg and "prompt" in error_msg:
                    logger.debug("Both conversation and prompt not supported, using core params only")
                elif "conversation" in error_msg:
                    try:
                        logger.debug("Retrying without conversation parameter")
                        return await self._client.responses.create(**core_params, prompt=self._non_null_or_not_given(prompt))
                    except TypeError as e2:
                        if "prompt" in str(e2):
                            logger.debug("Prompt also not supported, using core params only")
                        else:
                            raise e2
                elif "prompt" in error_msg:
                    try:
                        logger.debug("Retrying without prompt parameter")
                        return await self._client.responses.create(**core_params, conversation=conversation_id)
                    except TypeError as e2:
                        if "conversation" in str(e2):
                            logger.debug("Conversation also not supported, using core params only")
                        else:
                            raise e2
                else:
                ...
                # For any other exception, check if it's related to optional parameters
                error_msg = str(e).lower()
                if "conversation" in error_msg or "prompt" in error_msg:
                    logger.debug(f"Optional parameter related error, falling back: {str(e)}")
                else:
                    # Re-raise if it's not related to optional parameters
                    raise

        # Fallback: call with core parameters only (older OpenAI versions)
        logger.debug("Calling responses.create with core parameters only")
        return await self._client.responses.create(**core_params)
